        ...
        { //construct the trial simplex
          //based upon the initial guess parameters
            ...
            std::transform(del.begin(), del.end(), del.begin(),
                std::bind2nd(std::divides<D>(), static_cast<D>(20) ));//'20' is picked assuming initial trail close to true
            ...
            std::transform(init.begin(), init.end(),
                xcentroid_old.begin(), std::bind2nd(std::multiplies<D>(), static_cast<D>(N + 1)));
      ...
      //optimization begins
        for (cnt = 0; cnt < iterations; ++cnt) {
            ...
            std::transform(xg.begin(), xg.end(), xg.begin(),
                std::bind2nd(std::divides<D>(), static_cast<D>(N)));
            ...
            std::vector<D> xr(N, 0);
            for (int i = 0; i < N; ++i)
                xr[i] = xg[i] + static_cast<D>(a * (xg[i] - x[xnp1][i]));
            //reflection, xr found
            ...
            //expansion:
                ...
                std::vector<D> xe(N, 0);
                for (int i = 0; i < N; ++i)
                    xe[i] = xr[i] + static_cast<D>(b * (xr[i] - xg[i]));
                if (f(xe) < fxr)
            ...
            //contraction:
                ...
                std::vector<D> xc(N, 0);
                for (int i = 0; i < N; ++i)
                    xc[i] = xg[i] + static_cast<D>(g * (x[xnp1][i] - xg[i]));
                if (f(xc) < vf[xnp1])
                    ...
                    for (int i = 0; i < x.size(); ++i) {
                        if (i != x1) {
                            for (int j = 0; j < N; ++j)
                                x[i][j] = x[x1][j] + static_cast<D>(h * (x[i][j] - x[x1][j]));
                        }
                    }
