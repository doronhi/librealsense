...
namespace librealsense
{
    ...
    private:
        ...
        std::set<int> findPixByNeighbor(bool is_horizontal);
        std::set<int> setPixelsToCheck(bool is_horizontal);
        std::set<int> invalid_smear_dir(bool is_horizontal);
        void discard_low_gradients(bool is_horizontal);
        void calcDzAroundClosestEdge(const int x, const int y, bool is_horizontal, uint8_t& dz_around_edge, int& closestEdgeIx);
